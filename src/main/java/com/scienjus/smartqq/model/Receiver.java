...
public class Receiver {
	...
	private static SmartQQClient client = new SmartQQClient(new MessageCallback() {
		...
		@Override
		public void onGroupMessage(GroupMessage msg, SmartQQClient client) {
			if (!working) {
				return;
			}
			try {
				System.out.println("[" + getTime() + "] [" + getGroupName(msg) + "] " + getGroupUserNick(msg) + "："
						+ msg.getContent());
			} catch (Exception e) {
				e.printStackTrace();
			}
		...
		public void onDiscussMessage(DiscussMessage msg) {
			...
			try {
				System.out.println("[" + getTime() + "] [" + getDiscussName(msg) + "] " + getDiscussUserNick(msg) + "："
						+ msg.getContent());
			} catch (Exception e) {
				e.printStackTrace();
			}
